...
int main(int argc, char *argv[])
{
    ...
    f3=f4++; //operator++(int i)
    ++f4;    //operator++()
    f1=2+f2+3;  //operator+(const int &i, const Fecha &f) y operator+(const int &i) const
    cout << "Fechas: ";
    f1.ver();
    cout << ", ";
    f2.ver();
    ...
    cout << p->getDni() << " - " << c.getNombre() << ": " << j.getFecha() << endl; //j.getFecha() devuelve una variable tipo Fecha por lo que
                                                                                   //es necesario implementar el operator<< de la clase Fecha

    cout << *p << "\n" << c << "\n" << j << "\n";   //operator<< de la clase Cliente
    c = *p;
    p->setNombre("Susanita");
    p->setFecha(p->getFecha()+10);//operator+ / f+f(10)
    ...
    */
    ...
    Yoigo.ver();
    Yoigo.descuento(20);
    cout << "\nTras rebajar un 20% la tarifa de los ContratosMovil..."<<endl<<endl;
    Yoigo.ver();
